...
const Post = () => {
  // State to control the visibility of the new post form
  const [showForm, setShowForm] = useState(false);
  // State for the new post form inputs, matching backend fields
  const [formData, setFormData] = useState({
    title: '',
    description: '',
    category: '',
    imageUrl: '',
    author: 'Anonymous', // Default author for new posts
  });

  // State to store the list of posts fetched from the backend
  const [posts, setPosts] = useState([
    // Manually added posts for initial display with author and createdAt
    {
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      // Ensure fetched posts also have a createdAt field (if not from backend, add current time)
      const fetchedPostsWithTime = res.data.map(post => ({
        ...post,
        createdAt: post.createdAt || new Date().toISOString(), // Use existing or add current time
        author: post.author || 'Anonymous', // Use existing or default to Anonymous
      }));

      // Combine manual posts with fetched posts
      // Filter out duplicates if _id is the same (fetched posts take precedence)
      const combinedPosts = [...fetchedPostsWithTime, ...posts.filter(
        manualPost => !fetchedPostsWithTime.some(fetchedPost => fetchedPost._id === manualPost._id)
      )];
      ...
      // If backend fails, still show manual posts that were initially loaded
      setPosts(posts);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Add createdAt timestamp and author to the data sent to the backend
      const postData = { ...formData, createdAt: new Date().toISOString() };
      ...
      setError("Failed to create post. Please check your input and server connection.");
    }
  };
